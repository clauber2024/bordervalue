...
- `outputs/final_border_value_2026/relatorio_recortes_combustiveis_transicao.md`: recortes de hidrogenio, amonia e combustiveis da transicao.
- `outputs/final_border_value_2026/drivers_combustiveis_transicao_ncm.csv`: NCMs e drivers dos combustiveis da transicao.
...
### Etapa 5: validacao de hidrogenio, amonia e combustiveis da transicao
...
- [ ] Validar se cada NCM do recorte pertence de fato ao universo de hidrogenio, amonia, SAF, metanol, etanol ou combustiveis maritimos.
...
Nota de escopo: a candidata nao classifica ocupacoes verdes por CBO. A leitura de emprego vem de CNAE/RAIS/SCN67/MIP, portanto mede emprego associado a setores expostos a TSB, nao ocupacao verde individual.

Checklist:

- [ ] Confirmar que RAIS 2024 e uma camada de emprego formal, nao medida de producao.
- [ ] Confirmar que a leitura de emprego TSB deriva de CNAE/RAIS/SCN67/MIP e nao de CBO.
- [ ] Verificar se dashboard, relatorio e workbook evitam afirmar que ha classificacao ocupacional de empregos verdes.
...
Expansao posterior recomendada: criar uma etapa CBO propria, fora do escopo entregue nesta candidata. Essa etapa deve carregar RAIS com abertura por CBO, criar `dim_cbo_green_skills` ou `dim_cbo_transicao`, classificar ocupacoes como diretamente verdes, habilitadoras, industriais em setores TSB ou convencionais em setores de transicao, e cruzar CNAE + CBO + municipio. Assim, ficam separadas duas perguntas: "emprego em setor verde/TSB", coberto pela leitura setorial atual, e "ocupacao verde por CBO", que exige metodologia adicional.
...
| 3 | Combustiveis da transicao |  |  |  | Aprovado / Ressalva / Revisar / Excluir |  |  | Alta / Media / Baixa |  |
...
| 6 | Expansao CBO/ocupacoes |  |  |  | Aprovado / Ressalva / Revisar / Excluir |  |  | Alta / Media / Baixa |  |
...
- [ ] A leitura de emprego estiver descrita como setorial, baseada em CNAE/RAIS/SCN67/MIP, sem classificacao ocupacional CBO.
...
5. Quais fontes complementares sao indispensaveis para classificar hidrogenio, amonia, SAF, metanol, etanol e combustiveis maritimos como renovaveis, verdes, azuis ou de baixa emissao?
...
8. A expansao por CBO deve ser tratada como frente metodologica posterior para ocupacoes verdes, separada da leitura atual de emprego setorial?
