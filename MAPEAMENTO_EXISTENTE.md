...
- Resolvido (2026-08-06): fonte real identificada e conectada. A ANP publica um xlsx baixavel (nao so o Painel Dinamico Power BI sem export) em `gov.br/anp/pt-br/assuntos/renovabio/arq/certificacoes/certificados-aprovados-producao.xlsx`, atualizado quase diariamente (curl direto da 403 -- e o anti-bot do CDN gov.br, nao bloqueio de conteudo; precisa de headers de navegador). Snapshot 2026-08-06 salvo em `inputs/official/ANP_RenovaBio_CertificadosProducaoAprovados_2026-08-06.xlsx`.
  - `build_renovacalc_factors.py` (novo) parseia a aba "Validos" (527 certificados ativos) e carrega `analytical_renovacalc_certification` -- grao (processo_anp, razao_social, unidade_produtora, cidade, uf, cnpj, biocombustivel, rota_descricao, rota_codigo, conceptual_product_id, volume_elegivel_fracao, ano_base_certificacao, validade_certificado, fonte).
  - Crosswalk NCM -> rota confirmado contra `build_sector_sovereignty_metrics.py::CHAINS["combustiveis_transicao"]` (fonte real da camada Published, nao `lib/conceptualCatalog.ts`): `conceptual_product_id="etanol"` recebe todos os certificados Etanol hidratado/anidro (NCM 22071010/22071090/22072010/22072019), independente da rota (E1GC/E1GM/E1GMI/E1G2G/E1GFlex); `conceptual_product_id="biodiesel"` recebe Biodiesel (NCM 38260000). Biometano fica **fora** de proposito -- so existe como "gas_natural_biometano", um proxy fossil-dominante documentado la mesmo, e aplicar volume-elegivel de planta certificada a essa cesta (majoritariamente gas fossil) distorceria o indicador em vez de isolar a fatia real.
  - `sql/mv_published_indicators.sql` ganhou uma CTE `aggregated_renovacalc` (media nao ponderada do volume elegivel entre certificados ativos por `conceptual_product_id` -- mesmo metodo do proprio Painel Dinamico da ANP por rota, validado batendo o agregado deste loader para milho, 70,45%, contra o E1GM=70,15% publicado) e passou a expor `fator_alpha`, `fator_proporcionalidade_aplicado` e `fonte_proxy`.
  - `database/data_access.py` **nao precisou de nenhuma mudanca**: `_map_published_row` ja lia essas 3 colunas do resultado de `SELECT * FROM mv_published_indicators` de forma generica (`_first_present`/defaults), so nunca havia dado real fluindo ate agora. Validado ponta a ponta contra o Postgres de teste: `etanol` retorna `fator_alpha=0.8596`, `aplicado=True`, `fonte_proxy` citando a contagem de certificados; produtos sem match (ex.: `hidrogenio`) continuam caindo no default generico existente.
  - Gap conhecido e documentado no proprio loader: `cidade`/`uf` (parse best-effort do campo unico "Razao Social - Cidade - UF") resolvem para ~67% das linhas por inconsistencia de formatacao na fonte -- nao bloqueia o fator_alpha, que nao depende desses campos.
...
- Investigacao da fonte, parte 3 -- teste empirico do NEREUS contra o bridge CNAE do proprio projeto (2026-08-06): baixado `MIP-BR-CN10-68S-2018.xlsx` (usp.br/nereus, 954.828 bytes, confere com o Content-Length do servidor) e inspecionado com `openpyxl`. Confirma 68 atividades x 128 produtos, ano 2018, estimado em marco/2021 com base no SCN-IBGE de nov/2020. A planilha `Producao` (matriz de producao atividade x produto) foi cruzada, produto a produto, contra `outputs/tsb_bridge_2026/bridge_tsb_ncm.csv` (fonte real do `cnae_class_list` usado em `load_green_jobs()` para o split equal-weight hoje):
  - **fertilizantes**: `amonia`, `ureia`, `sulfato_amonio`, `superfosfatos`, `cloreto_potassio` e `fosfato_monoamonico` compartilham o mesmo `cnae_class_list=2012` no bridge do projeto (colisao real de 6 produtos conceituais em 1 CNAE, hoje resolvida por equal-weight `1/6`). No NEREUS, todos esses produtos caem na mesma linha de produto `20912 "Adubos e fertilizantes"` -- **nao ha nenhuma linha mais fina que separe ureia de sulfato de amonio de superfosfato**. O nivel 128-produtos do NEREUS para de dividir exatamente onde o CNAE do projeto ja para.
  - **aco**: `planos_quente` e `planos_frios` compartilham `cnae_class_list=2422` (colisao real, equal-weight `1/2` hoje). No NEREUS ambos caem na mesma linha de produto `24912 "Semi-acabados, laminados planos, longos e tubos de aco"` -- mesma limitacao. (`ferro_gusa` e `ferroligas` tem CNAEs distintos no bridge do projeto -- 2411 vs 2412 -- e portanto ja nao colidem hoje; o NEREUS ainda assim os fundiria em uma unica linha `24911`, ou seja e mais grosseiro que o bridge atual nesse ponto especifico.)
  - **silicio**: nao ha nenhuma linha de produto dedicada a silicio metalico nas 128 -- fica implicito dentro de `24921 "Produtos da metalurgia de metais nao-ferrosos"`, junto de aluminio, cobre etc. Pior granularidade que as outras duas cadeias.
  - Conclusao: o teto de granularidade do NEREUS (128 produtos, nivel SCN) e o mesmo teto do bridge CNAE que o projeto ja usa (ou mais grosseiro, no caso do aco). Construir o crosswalk CNAE -> atividade MIP68 nao adicionaria poder de desagregacao nas colisoes reais que existem hoje -- so trocaria uma aproximacao equal-weight (interna, sem defasagem, documentada) por uma aproximacao equal-weight equivalente vinda de terceiros (2018, 8 anos de defasagem, sem corte municipal), sem ganho de precisao.
- Classificacao: **limitacao estrutural confirmada empiricamente, nao mitigada pelo NEREUS** -- decisao (2026-08-06): **nao implementar** o crosswalk/rateio via NEREUS. O teste produto-a-produto acima mostra que a fonte de terceiros mais promissora (NEREUS nivel 68) nao teria poder de desagregacao nos pontos reais de colisao do pipeline (fertilizantes CNAE 2012, aco CNAE 2422), so trocaria uma aproximacao interna e atual por uma aproximacao equivalente e defasada. Diferente do RenovaCalc (dado primario oficial, atual, plant-level, com ganho de precisao real e verificado), aqui nao existe fonte -- oficial ou academica -- que resolva a granularidade abaixo do CNAE 4-digitos para estes 3 produtos. Manter `proportion_factor=1.0` como aproximacao documentada, nao como bug pendente. Arquivo baixado preservado em `C:\Users\polis\AppData\Local\Temp\claude\E--\fdf88514-5133-4115-a588-64738ca84c28\scratchpad\nereus\MIP-BR-CN10-68S-2018.xlsx` para re-inspecao se necessario; nao commitado ao repo.
- Fix decoupled resolvido (2026-08-06): `database/data_access.py` (linha ~526, deslocada pela mudanca do RenovaCalc) trocou o default de `fonte_proxy` de `"Matriz Insumo-Produto IBGE"` para `"Aproximacao equal-weight (sem MIP -- ver MAPEAMENTO_EXISTENTE.md)"`. So esse ponto foi tocado -- o mock estatico em `api/main.py:361` (`DATABASE_MOCK`, ja classificado como prototipo puro em "Dados mockados" acima) e o fixture equivalente em `tests/test_transition_fuels_api_contract.py:150` (dado sintetico de teste, nao asserção sobre o texto) ficaram como estavam, por serem dados falsos de qualquer forma -- nao precisam do rotulo correto porque nao fingem vir da camada Published.
