...
const glass = "backdrop-blur-xl bg-surface-1/40 border border-border/[0.08] shadow-2xl";
...
export function ConceptualProductCard({ product, isLoading = false, error, onRetry }: ConceptualProductCardProps) {
  ...
  return (
    <article className={`${glass} overflow-hidden rounded-2xl`}>
      <div className="p-5 sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <h3 className="mt-4 text-2xl font-bold tracking-tight text-ink-heading">{product.name}</h3>
            <p className="mt-2 text-sm leading-6 text-ink-muted">{product.shortDescription}</p>
          </div>

          <div className="rounded-xl border border-border/[0.08] bg-border/[0.04] px-4 py-3 sm:min-w-40">
            <p className="flex items-center gap-2 text-xs font-medium uppercase tracking-[0.16em] text-ink-faint">
              <Factory className="h-3.5 w-3.5" strokeWidth={1.5} />
              Etapa
            </p>
            <p className="mt-2 text-sm font-bold text-ink-body">{product.productionStage}</p>
          </div>
        </div>
      ...
      <div className="border-t border-border/[0.08] bg-surface-0/25">
        <button
          type="button"
          aria-expanded={open}
          aria-controls={panelId}
          onClick={() => setOpen((value) => !value)}
          className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left text-sm font-medium text-ink-body outline-none transition hover:bg-border/[0.04] focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-cyan-300 sm:px-6"
        >
          ...
          {open ? (
            ...
            >
              <div className="space-y-5 px-5 pb-6 sm:px-6">
                <div>
                  <h4 className="text-sm font-medium text-ink-body">Fontes</h4>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {product.sources.map((source) => (
                      <span key={source} className="rounded-full border border-border/[0.08] bg-border/[0.04] px-2.5 py-1 text-xs text-ink-body">
                        {source}
                      </span>
                    ))}
                  </div>
                ...
                {product.methodology ? (
                  <div>
                    <h4 className="text-sm font-medium text-ink-body">Metodologia</h4>
                    <p className="mt-2 text-sm leading-6 text-ink-muted">{product.methodology}</p>
                  </div>
                ) : null}
...
function Metric({
  ...
  return (
    <div className="rounded-xl border border-border/[0.08] bg-border/[0.04] p-4">
      ...
      <strong className="mt-3 block truncate text-lg font-bold tracking-tight text-ink-heading">{value}</strong>
    </div>
  );
}
  ...
  return (
    <article className={`${glass} rounded-2xl p-5 sm:p-6`}>
      <div className="animate-pulse space-y-5">
        <div className="h-6 w-48 rounded-full bg-border/[0.08]" />
        <div className="h-8 w-2/3 rounded-lg bg-border/[0.08]" />
        <div className="h-4 w-full rounded bg-border/[0.06]" />
        <div className="h-4 w-4/5 rounded bg-border/[0.06]" />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, index) => (
            <div key={index} className="h-24 rounded-xl border border-border/[0.08] bg-border/[0.04]" />
          ))}
        </div>
      </div>
    </article>
...
export function ConceptualProductCardError({ error, onRetry }: { error: string; onRetry?: () => void }) {
  return (
    <article className={`${glass} rounded-2xl p-6 text-center`}>
      ...
      <h3 className="mt-4 text-lg font-bold tracking-tight text-ink-heading">Produto indisponível</h3>
      <p className="mt-2 text-sm text-ink-muted">{error}</p>
      {onRetry ? (
        <button
          type="button"
          onClick={onRetry}
          className="mt-5 rounded-lg border border-border/[0.1] bg-border/[0.05] px-4 py-2 text-sm font-medium text-ink-heading outline-none transition hover:bg-border/[0.1] focus-visible:ring-2 focus-visible:ring-cyan-300"
        >
          Recarregar
        </button>
      ) : null}
...
export function ConceptualProductCardEmpty() {
  return (
    <article className={`${glass} rounded-2xl p-6 text-center`}>
      ...
      <h3 className="mt-4 text-lg font-bold tracking-tight text-ink-heading">Nenhum produto neste recorte</h3>
      <p className="mt-2 text-sm text-ink-muted">Tente ampliar o período ou remover um filtro técnico.</p>
    </article>
  );
}
