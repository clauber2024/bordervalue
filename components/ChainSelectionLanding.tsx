...
const UNKNOWN_RISK: RiskPresentation = {
  ...
  style: "bg-surface-1 text-ink-faint border-border",
};
  ...
  return (
    <section className="mx-auto max-w-none py-8 sm:py-12">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h1 className="mt-4 text-3xl font-bold tracking-tight text-ink-heading sm:text-5xl">
          Qual cadeia produtiva você quer analisar?
        </h1>
        <p className="mx-auto mt-4 max-w-2xl text-sm leading-6 text-ink-muted sm:text-base">
        ...
        <nav aria-label="Recursos gerais da plataforma" className="mt-6 flex flex-wrap justify-center gap-2">
          <Link
            href="/explorar"
            className="inline-flex items-center gap-2 rounded-lg border border-border/10 bg-border/[0.04] px-3 py-2 text-xs font-semibold text-ink-body transition hover:border-cyan-300/25 hover:bg-cyan-400/[0.07] hover:text-ink-heading"
          >
          ...
          <Link
            href="/tour-soberania"
            className="inline-flex items-center gap-2 rounded-lg border border-border/10 bg-border/[0.04] px-3 py-2 text-xs font-semibold text-ink-body transition hover:border-emerald-300/25 hover:bg-emerald-400/[0.07] hover:text-ink-heading"
          >
      ...
      <div className="mx-auto mt-10 flex max-w-5xl flex-col items-center gap-3 rounded-2xl border border-border/[0.08] bg-surface-1/40 p-3 backdrop-blur-xl sm:flex-row sm:justify-between">
        ...
        <label className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="h-9 w-full rounded-xl border border-border bg-surface-0/80 pl-9 pr-3 font-mono text-xs text-ink-body placeholder-ink-faint outline-none transition focus:border-cyan-400/50 focus:ring-2 focus:ring-cyan-400/20"
          />
        </label>
      </div>
      ...
      {isLoading ? (
        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {Array.from({ length: 8 }).map((_, index) => (
            <div key={index} className="h-64 animate-pulse rounded-2xl border border-border/5 bg-surface-1/40" />
          ))}
        </div>
      ) : (
        <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
...
function MacroStat({ label, value, tone = "text-ink-body" }: { label: string; value: string; tone?: string }) {
  return (
    <div className="rounded-xl border border-border/80 bg-surface-1/40 p-3 backdrop-blur-md">
  ...
  return (
    <button
      ...
      className={`whitespace-nowrap rounded-xl px-3 py-1.5 text-xs font-semibold transition ${
        active
          ? "border border-border bg-surface-1 text-emerald-400 shadow-md"
          : "border border-transparent text-ink-muted hover:text-ink-body"
      }`}
    >
      {label}
    </button>
  ...
  return (
    <div
      className={`group flex flex-col justify-between rounded-2xl border p-5 backdrop-blur-xl transition ${
        isPublished
          ? `border-border/[0.08] bg-surface-1/40 shadow-xl ${
          ...
          : "border-border bg-surface-0/40 opacity-60"
      }`}
    >
      <div>
        <div className="flex items-center justify-between gap-2">
          <span className="rounded-md border border-border bg-surface-1 px-2 py-0.5 font-mono text-[10px] text-ink-muted">
            {chain.group}
          </span>
          <span className={`rounded-full border px-2 py-0.5 font-mono text-[10px] font-bold ${isPublished ? risk.style : "border-border bg-surface-1 text-ink-faint"}`}>
            {isPublished ? risk.label : "Em Homologação"}
          </span>
        </div>

        <div className="mt-3 flex items-center gap-2">
          <Database className={`h-4 w-4 ${isPublished ? "text-cyan-300" : "text-ink-faint"}`} />
          <h3 className="text-base font-bold text-ink-heading transition group-hover:text-cyan-300">{chain.name}</h3>
        </div>
        <p className="mt-2 text-xs leading-5 text-ink-muted">{chain.description}</p>
      </div>

      <div className="mt-5">
        {isPublished && summary?.ok ? (
          <div className="mb-4 space-y-1.5 rounded-xl border border-border/80 bg-surface-0/60 p-3 font-mono text-[11px]">
            <div className="flex items-center justify-between text-ink-muted">
              <span>Insumos monitorados:</span>
              <strong className="text-ink-body">{summary.inputsCount} itens</strong>
            </div>
            <div className="flex items-center justify-between text-ink-muted">
            ...
            {summary.topBottleneck ? (
              <div className="truncate border-t border-border pt-1.5 text-ink-muted">
                <span className="text-ink-faint">Gargalo:</span> {summary.topBottleneck}
              </div>
            ) : null}
          ...
          <div className="mb-4 flex items-center gap-2 rounded-xl border border-border/80 bg-surface-0/60 p-3 font-mono text-[11px] text-ink-faint">
        ...
        {isPublished ? (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-surface-1 px-4 py-2.5 text-xs font-semibold text-emerald-400 transition hover:border-emerald-400 hover:bg-emerald-500 hover:text-zinc-950"
          >
          ...
          <button
            type="button"
            disabled
            className="w-full cursor-not-allowed rounded-xl border border-border bg-surface-0 px-4 py-2.5 text-center font-mono text-xs text-ink-faint"
          >
            Base em Mapeamento
          </button>
        )}
